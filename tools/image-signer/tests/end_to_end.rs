...
//!
//! A payload is signed with the all-0x01 dev seed, then the SAME bytes are
...
// The all-0x01 dev seed. Its public key equals fw_update::DEV_ROOT_KEY.
...
{
    let signer = SoftwareSigner::from_seed(&DEV_SEED);
    ...
    // 2. The signer's public key must be the dev root key the firmware pins.
    assert_eq!(signer.public_key(), fw_update::DEV_ROOT_KEY);

    // 3. image-verify accepts the exact bytes under the dev root key.
    let root = RootKey::from_bytes(fw_update::DEV_ROOT_KEY)
        .expect("dev root on-curve");
    let verified =
        verify_image(&image, &root).expect("verify_image must accept");
    assert_eq!(verified.payload(), payload);
    assert_eq!(verified.security_counter(), security_counter);

    // 4. The dual-bank update machine consumes the SAME bytes through its mock
    //    seam, all the way to a committed and confirmed swap.
    ...
    // The machine streamed the exact tool output into its inactive bank, then
    // ran verify off that same bank. Reaching PendingCommit is the public proof
    // the machine accepted the signer's real bytes (it is non-vacuous: the
    // wrong-key case below reaches an Err on this same call).
...
{
    // A non-dev seed yields a different public key, so the machine pinning the
    // dev root key must reject the tool's output. This pins that the e2e accept
    // above is real, not a path that accepts anything.
    let signer = SoftwareSigner::from_seed(&[2u8; 32]);
